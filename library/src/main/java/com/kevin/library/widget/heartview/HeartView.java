...
/**
 * Class: HeartView </br>
 * Description: 心形的view </br>
 * Creator: kevin </br>
 * Email: jiangtao103cp@gmail.com </br>
 * Date: 16/9/18 下午11:10</br>
 * Update: 16/9/18 下午11:10 </br>
 **/
public class HeartView extends SurfaceView implements SurfaceHolder.Callback {
  ...
  public Point getHeartPoint(float angle) {
    ...
    float y = (float) (-heartRadio * (13 * Math.cos(t)
        - 5 * Math.cos(2 * t)
        - 2 * Math.cos(3 * t)
        - Math.cos(4 * t)));
  ...
  private void drawOnNewThread() {
    if (mHeartViewCallBack != null) mHeartViewCallBack.onHeartStart();
    new Thread() {
      @Override public void run() {
        ...
        isDrawing = false;
        if (mHeartViewCallBack != null) mHeartViewCallBack.onHeartFinish();
      }
    }.start();
  }
  ...
  public void setHeartViewCallBack(HeartViewCallBack callBack) {
    mHeartViewCallBack = callBack;
  }
}